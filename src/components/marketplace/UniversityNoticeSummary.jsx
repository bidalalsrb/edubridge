  ...
  return (
    <div className="mt-5 grid gap-3">
      {notices.map((notice) => (
        <Card
          key={notice.id}
          className={`relative bg-white ${notice.isMine ? 'border-red-400' : 'border-brand-200'} ${onOpenNotice ? 'cursor-pointer transition hover:border-brand-600' : ''}`}
          ...
          <CardContent className="p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                {notice.isMine ? (
                  <span className="rounded-full bg-brand-600 px-2 py-1 text-xs font-bold text-white">내가 등록한 공고</span>
                ) : null}
