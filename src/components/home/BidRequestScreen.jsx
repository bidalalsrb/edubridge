    ...
    <div className="bg-white">
      <header className="flex items-center gap-4 border-b border-slate-100 px-6 py-5">
        ...
        <div>
          ...
          <p className="text-[22px] font-semibold tracking-[-0.03em] text-[#336fea]">{directionText}</p>
        </div>
      </header>

      <section className="space-y-6 bg-[#f8f9fc] px-6 py-6">
        <div className="rounded-2xl bg-[#edf4ff] px-4 py-3 text-[24px] font-bold tracking-[-0.03em] text-[#336fea]">
            ...
            {categoryOptions.map((option, idx) => (
              <button
                ...
                className={cn(
                  ...
                  idx === 0 ? 'bg-[#336fea] text-white' : 'bg-slate-200 text-slate-600',
                )}
                type="button"
              >
                {option}
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-[#d7dfea] bg-white px-4 py-4">
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-2xl border border-[#d7dfea] bg-white px-4 py-4 text-[28px] font-semibold tracking-[-0.03em] text-slate-400">
            행사 목적, 대상, 주요 프로그램 등 입찰 업체가 참고할 상세 내용을 입력해 주세요.
          </div>
        </div>

        <div className="flex items-start gap-2 rounded-2xl border border-[#d4e3fb] bg-[#edf4ff] px-4 py-4">
          <CircleAlert className="mt-0.5 h-5 w-5 text-[#336fea]" />
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#336fea] px-6 py-4 text-[36px] font-black tracking-[-0.05em] text-white"
        >
